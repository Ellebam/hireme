...
|---|------|-------|----------|-------------|
| 1 | Parse* function unit tests | Backend `section.go` | High | Create `section_test.go` with dedicated tests for each `Parse*` function (`ParsePersonal`, `ParseSummary`, `ParseExperience`, `ParseEducation`, `ParseSkills`, `ParseLanguages`). Test: valid JSON → correct struct, malformed JSON → zero-value struct (not nil), empty `json.RawMessage` → zero-value struct. These are the foundation; a silent parse failure would produce blank sections. Use table-driven tests per function. |
| 2 | formatDate template function | Backend `renderer_test.go` | High | Test the `formatDate` helper independently: `"2020-01"` → `"Jan 2020"`, `"2017-06"` → `"Jun 2017"`, year-only `"2020"` → `"2020"`, empty string → `""`, malformed input → graceful fallback. Date formatting bugs affect every template's experience and education sections. Add as a `TestFormatDate` function at the top of `renderer_test.go`. |
| 3 | Section ordering verification | Backend `renderer_test.go` | High | Supply sections in **reverse order** (e.g., languages order=5 first, personal order=0 last) and verify the rendered HTML outputs them in correct ascending order. The renderer's `TemplateData` building must sort visible sections by `Order`. Without this, shuffled input silently produces a misordered CV. Add as a test case within the per-template test suite. |
| 4 | Proficiency-to-level mapping (Modern) | Backend `renderer_test.go` | High | The Modern template renders a 5-bar chart for languages. Test that each proficiency level maps correctly: `native`→5, `fluent`→4, `advanced`→3, `intermediate`→2, `basic`→1, `unknown`→0 (or fallback). Verify the HTML output contains the correct number of filled vs empty bars. This is a key visual feature unique to the Modern template. |
| 5 | New domain types JSON round-trip | Backend `domain/cv_test.go` | Medium | Add tests for the new `SummaryContent`, `LanguagesContent`, and `LanguageEntry` types: marshal → unmarshal round-trip, verify `omitempty` on `Certification` field, verify `LanguagesContent.Entries` deserializes correctly from the seed data JSON structure. Add after `TestCV_SetContent_WithStyling` (~L179). |
| 6 | Visionary sidebar routing | Backend `renderer_test.go` | Medium | The Visionary template splits sections into sidebar (personal, skills, languages) and main (summary, experience, education). Render a full CV with the `visionary` template and verify: sidebar HTML region contains personal/skills/languages content, main region contains summary/experience/education. Ensures the Go template replicates the React `SIDEBAR_TYPES` set logic. |
| 7 | Font family CSS mapping | Backend `renderer_test.go` | Medium | Test that styling `fontFamily: "inter"` produces the correct CSS font stack (e.g., `"Inter", system-ui, -apple-system, sans-serif`) in the rendered `<style>` block. Also test fallback when `fontFamily` is empty/unknown → default font stack. Add as a test case within the custom styling edge case tests. |
| 8 | TemplateData building from CVContent | Backend `renderer_test.go` or `section_test.go` | Medium | Test the intermediate step where `CVContent` (with `json.RawMessage` sections) is transformed into `TemplateData` (with typed content). Supply a CVContent with all 6 section types and verify each typed field in TemplateData is populated correctly. This is the integration point between parsing and rendering; a bug here causes content to silently disappear. If `buildTemplateData` is exported or testable, test directly; otherwise verify through rendered output. |
| 9 | NewRenderer initialization | Backend `renderer_test.go` | Low | Verify that `NewRenderer()` succeeds (no error) under normal conditions. A simple `TestNewRenderer_Success` asserting `err == nil` and `renderer != nil`. Confirms the `//go:embed` + `template.ParseFS` pipeline works. |
| 10 | Cross-template structural consistency | Backend `renderer_test.go` | Low | Render the same full CV content through all 3 templates and verify each output contains the same data points (name, job title, company names, skill names, etc.), just formatted differently. This catches regressions where one template accidentally drops a section. Can be a single test with a loop over template IDs. |
